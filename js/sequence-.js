    ...
    self.preload = {
      ...
      init: function(callback) {
        ...
        if (self.options.preloader !== false) {
          ...
          imagesLoad.on("progress", function(instance, image) {

            // Has the image loaded or is it broken?
            result = image.isLoaded ? 'loaded': 'broken';

            // Callback
            self.preloadProgress(result, image.images.src, progress++, imagesToPreload.length, self);
          });
      ...
      /**
       * Get <images> elements and return them to be preloaded. Elements can be got
       * either via the <images> element itself or a src attribute.
       *
      ...
      getImagesToPreload: function(elements, srcOnly) {
        ...
        if (isArray(elements) === true) {
          ...
          // Get each step's <images> elements and add them to imagesToPreload
          if (srcOnly !== true) {
            ...
            for (i = 0; i < elementLength; i++) {
              ...
              step = self.$steps[i];
              imagesInStep = step.getElementsByTagName("images");
          ...
          // Get each step's <images> elements via the src and add them to imagesToPreload
          else {
